CLASS zcl_medapi_tree_model DEFINITION
  ...
  PRIVATE SECTION.
    ...
    TYPES:
      ...
      ty_namespace_models TYPE SORTED TABLE OF ty_namespace_model WITH UNIQUE KEY namespace.

    DATA mt_models TYPE ty_namespace_models.
...
CLASS zcl_medapi_tree_model IMPLEMENTATION.
  ...
  METHOD if_ish_gui_table_model~get_entries.
    ...
    rt_entry = VALUE #( FOR ls_model IN mt_models
                        ( ls_model-model ) ).
  ...
  METHOD add_class.
    ...
    IF <ls_model> IS NOT ASSIGNED.
      INSERT VALUE #( namespace = lv_namespace
                      model     = build_namespace_model( iv_namespace  = lv_namespace
                                                         iv_class_name = iv_class_name ) )
             INTO TABLE mt_models ASSIGNING <ls_model>.
    ENDIF.

    DATA(ls_api_list) =
        VALUE zmedapi_s_list( LET li_api = cl_ishmed_api_factory=>get_instance( iv_class_name ) IN
  ...
  METHOD build_namespace_model.
    ...
    CASE iv_namespace.
      ...
      WHEN gc_gns_sap_all.
        lv_node_text = COND #( WHEN iv_class_name CS `ISHMED`
                               THEN |i.s.h.med ({ 'By'(001) } Cerner)|
      ...
      WHEN OTHERS.
        ...
        lv_node_text =
            COND #(
                LET l_description = COND #( WHEN ls_text-descriptn IS NOT INITIAL
                                            THEN ls_text-descriptn
    ...
    DATA(lv_node_icon) = SWITCH tv_image( iv_namespace
                                          WHEN gc_gns_sap_all THEN `ICON_SAP`
                                          WHEN gc_gns_cust
                                            OR space          THEN `ICON_CUSTOMER`
                                          ELSE                     `ICON_PARTNER` ).

    ri_result = cl_ish_gm_table_simple=>create( i_node_text = lv_node_text
                                                i_node_icon = lv_node_icon ).
